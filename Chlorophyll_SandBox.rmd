...
product_title <- "SST gradients 2001-2018 trends and anomalies"
short_description <- "Sea surface temperature is the temperature of the water close to the sea surface. SST is a standard product from satellite-based thermal infra-red sensors, and optical sensors complemented with infrared bands. The gradients are processed from Monthly mean sea surface temperature (in degree-C at 4km resolution) derived from the MODIS-TERRA sensor (Satellite remote sensing Ocean color data)."
...
#http://elk.knowcean.eu/elk/chla_2016-01-01/_search
#nginx rule :80/elk > :9200 only 'search'
#url_elk <- 'http://elk.knowcean.eu:9200/chla_'
#url_elksst <- 'http://elk.knowcean.eu:9200/sst_'
#url_elksst <- 'http://192.168.0.155:9200/sst_'
#url_elk <- 'http://192.168.0.155:9200/chla_'

#http://elk.knowcean.eu/elk/sst_2020-02-01/_search?
url_elk <- 'http://elk.knowcean.eu/elk/chla_'
url_elksst <- 'http://elk.knowcean.eu/elk/sst_'
...
p1 <- ggplot(dat,aes(x=gradient,color=gradient,after_stat(count))) + geom_density() + ggtitle(paste0("Year: ",year," Month: ",month))
plot(p1)
...
p1 <- ggplot(dat,aes(x=gradient,color=gradient,after_stat(count))) + geom_density() + ggtitle(paste0("Log10 Year: ",year," Month: ",month))
plot(p1)
